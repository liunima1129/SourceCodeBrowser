...
namespace Utilities
{
	...
	inline ProcessCmdLine::Patterns ProcessCmdLine::getPatts()
	{
		return patterns_;
	}

	inline ProcessCmdLine::Regexes ProcessCmdLine::getRegxs()
	{
		return regexes_;
	}
	...
	template <typename T>
	std::string Converter_2<T>::toString(const T& t)
	{
	...
	*/
	template<typename T>
	T Converter_2<T>::toValue(const std::string& src)
	{
