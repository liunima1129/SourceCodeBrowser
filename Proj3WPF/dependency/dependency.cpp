...
void Dependency::setTree(std::vector<std::string> fileTree, std::string outpath_)
{
	for (auto files : fileTree)
		treeTraverse(files, outpath_);
}

void Dependency::treeTraverse(const std::string& root, std::string outpath_)
{
	...
	_DependFiles = includeFiles(root, _DependFiles);
	_dependencyTree.push_back(root);
	toggleDisplay _generate;
	_generate.htmlGenerate(root, _DependFiles, tables, outpath_);
